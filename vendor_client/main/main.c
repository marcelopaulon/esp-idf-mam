...
int msg_entry_compare(const void *a, const void *b, void *udata) {
    const struct msgEntry *ua = a;
    const struct msgEntry *ub = b;
    return strcmp(ua->key, ub->key);
}

bool msg_entry_iter(const void *item, void *udata) {
    const struct msgEntry *msgEntry = item;
    printf("%s (count=%u)\n", msgEntry->key, msgEntry->count);
    return true;
}

uint64_t msg_entry_hash(const void *item, uint64_t seed0, uint64_t seed1) {
    const struct msgEntry *msgEntry = item;
    return hashmap_sip(msgEntry->key, strlen(msgEntry->key), seed0, seed1);
}
...
void app_main(void)
{
    ...
    map = hashmap_new(sizeof(struct msgEntry), 0, 0, 0, 
                                     msg_entry_hash, msg_entry_compare, NULL, NULL);
